...
with sync_playwright() as p:
    ...
    # collapse all by clicking first header (which toggles open then we close)
    page.locator('.wa-creator-field-header').first.click()
    page.wait_for_timeout(300)
    page.locator('.wa-creator-field-header').first.click()
    page.wait_for_timeout(300)

    # set scroll so header 3 is visible near top
