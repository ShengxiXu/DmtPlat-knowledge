from playwright.sync_api import sync_playwright

BASE_URL = 'http://localhost:5173'
with sync_playwright() as p:
    browser = p.chromium.launch(headless=True)
    page = browser.new_page(viewport={'width': 1440, 'height': 900})
    page.expose_function('py_log', lambda *args: print('[page]', *args))
    page.goto(f'{BASE_URL}?view=workAssistant')
    page.wait_for_timeout(1000)
    # override prototype before entering creator
    page.evaluate('''() => {
        const orig = WorkAssistant.prototype.renderTemplateCreator;
        WorkAssistant.prototype.renderTemplateCreator = function(...args) {
            const el = document.querySelector('.wa-creator-form-config');
            window.py_log('snapshot before render', el ? el.scrollTop : null);
            const r = orig.apply(this, args);
            const el2 = document.querySelector('.wa-creator-form-config');
            window.py_log('scroll after render', el2 ? el2.scrollTop : null);
            return r;
        };
    }''')
    page.locator('#wa-template-creator-btn').click()
    page.wait_for_timeout(500)
    page.locator('.wa-creator-tab[data-tab="form"]').click()
    page.wait_for_timeout(300)
    page.locator('#creator-name').fill('测试')

    for i in range(8):
        page.locator('#wa-creator-add-field').click()
        page.wait_for_timeout(200)
        labels = page.locator('.wa-creator-field-label').all()
        labels[-1].fill(f'字段{i+1}')
        labels[-1].blur()
        page.wait_for_timeout(200)

    page.locator('.wa-creator-field-header').first.click()
    page.wait_for_timeout(300)
    page.evaluate('document.querySelector(".wa-creator-form-config").scrollTop = 300')
    print('initial scroll', page.evaluate('document.querySelector(".wa-creator-form-config").scrollTop'))

    headers = page.locator('.wa-creator-field-header').all()
    headers[4].click()
    page.wait_for_timeout(500)
    print('final scroll', page.evaluate('document.querySelector(".wa-creator-form-config").scrollTop'))
    browser.close()
