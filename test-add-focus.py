...
with sync_playwright() as p:
    ...
    print('before add scroll', page.evaluate('document.querySelector(".wa-creator-form-config").scrollTop'))
    page.locator('#wa-creator-add-field').click(force=True)
    page.wait_for_timeout(500)
    print('after add scroll', page.evaluate('document.querySelector(".wa-creator-form-config").scrollTop'))
    print('active element', page.evaluate('''() => {
        const el = document.activeElement;
        return el ? {tag: el.tagName, className: el.className, id: el.id, dataset: el.dataset} : null;
    }'''))
    browser.close()
