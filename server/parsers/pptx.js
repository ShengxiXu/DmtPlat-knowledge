...
function getSlideTexts(slideXml) {
  ...
  for (const node of shapeNodes) {
    ...
    const nvPr = node['p:nvSpPr']?.[0] || node['p:nvPicPr']?.[0] || node['p:nvGraphicFramePr']?.[0];
...
function classifySlideType(index, total, shapes) {
  ...
  // 内容页：有标题和正文
  const hasTitle = shapes.some((s) => s.placeholderType === 'title' || s.placeholderType === 'ctrTitle');
  const hasBody = shapes.some((s) => s.placeholderType === 'body' || s.placeholderType === 'obj');
  ...
  try {
    ...
    if (colorScheme) {
      const extractColor = (elem) => {
        const rgb = elem?.['a:srgbClr']?.[0]?.$?.val || elem?.['a:sysClr']?.[0]?.$?.lastClr;
        return rgb ? `#${rgb}` : null;
      };
...
export async function parsePPTX(fileBuffer) {
  ...
  const sldIdLst = pres['p:presentation']?.['p:sldIdLst']?.[0]?.['p:sldId'] || [];
  const slideRIds = sldIdLst.map((s) => s.$?.['r:id'] || s.$?.id).filter(Boolean);
  ...
  for (let i = 0; i < slideRIds.length; i++) {
    ...
    const titleShape = shapes.find((s) => s.placeholderType === 'title' || s.placeholderType === 'ctrTitle');
    const bodyShapes = shapes.filter((s) => s.placeholderType === 'body' || s.placeholderType === 'obj' || !s.placeholderType);
...
function buildPPTSuggestedFields(slides, theme) {
  const fields = [
    { id: 'topic', label: '演讲主题', type: 'text', required: true, placeholder: '例如：DmtPlat 产品发布会' },
    { id: 'audience', label: '受众', type: 'text', required: false, placeholder: '例如：潜在客户、销售团队' },
    { id: 'core_message', label: '核心信息', type: 'textarea', required: false, placeholder: '本次演讲要传递的 1-3 个核心观点', rows: 3 },
  ];
